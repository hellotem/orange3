...
class ClusterTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            return "{:.3f}".format(score) if valid else "缺失值"
...
class OWKSpectralClustering(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        ...
        no_attributes = widget.Msg("数据缺失特征。")

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg(
            "轮廓得分不计算大于 {} 个样本".format(
                SILHOUETTE_MAX_SAMPLES)
        )
        not_enough_data = widget.Msg(
            "对于 {} 个簇来说唯一的数据实例太少 ({})"
        )
        no_sparse_normalization = widget.Msg("稀疏数据无法归一化")

    INIT_METHODS = (("用 KMeans++ 初始化", "k-means++"),
                    ("随机初始化", "random"))
    ...
    def __init__(self):
        ...
        bg = gui.radioButtonsInBox(
            ...
            box="簇数", callback=self.update_method,
        )
